...
|---------|---------|-------------|---------------|--------|
| LC 1658 — Minimum Operations to Reduce X to Zero | Sliding Window — Variable Size (complement reframing) | 2026-08-05 | 2026-08-26 — standard fuse. Self-diagnosed the gap upfront (reframing, not window mechanics). Needed 4 guided questions to reach "find longest subarray summing to totalSum - x," then implemented the full solution independently with one small self-caught bug (leftover `return -1`). Verified against all examples plus 3,000 random tests. At redo, check specifically whether the reframing itself comes cold — the window mechanics are not in question. | ⏳ |
| LC 438 — Find All Anagrams in a String | Sliding Window — Fixed Size, exact-frequency Map match (multi-result) | 2026-08-04 | 2026-08-25 — standard fuse. Direct extension of LC 567, unprompted self-connected. Core sliding-window logic correct from the start (verified against 10,000 random tests); the actual bug was a copy-paste artifact (real logic left under the wrong function name, patterns.md #76) — self-caught in one question. No repeat of the LC 567 implicit-global bug. At redo, check whether the function-name check happens proactively when adapting a prior solution. | ⏳ |
| LC 567 — Permutation in String | Sliding Window — Fixed Size, exact-frequency Map match | 2026-08-04 | 2026-08-25 — standard fuse. Algorithm correct from the start (verified against 20,000 random tests). One recurring bug not self-caught (implicit global in `isSameMap`, patterns.md #10, 4th occurrence) — fixed once flagged. Unprompted made the fundamentals connection, with correct reasoning (Set vs Map on duplicates) but a small mislabel of which fundamentals problem matches. At redo, check whether the implicit-global habit and the fundamentals-problem-name precision have both improved. | ⏳ |
| LC 1456 — Maximum Number of Vowels in a Substring of Given Length | Sliding Window — Fixed Size (Set lookup + count slide) | 2026-08-04 | 2026-08-25 — standard fuse. Clean, self-written, correct on one submission — unprompted self-connected to the prior two problems before any code shown. Two complexity slips (O(k) vs O(1) space, "O(N-K)" vs O(N) time), both self-corrected in one beat. At redo, check whether the first-stated complexity is correct without needing the correction step at all. | ⏳ |
| LC 1343 — Number of Sub-arrays of Size K and Average ≥ Threshold | Sliding Window — Fixed Size (numeric slide + count) | 2026-08-04 | 2026-08-25 — standard fuse. Self-explained the approach unprompted and unprompted named the LC 643 connection. One real bug (slide loop bound `arr.length` instead of `arr.length - k`, masked by JS's `NaN` propagation rather than crashing) — self-corrected in one question. At redo, check whether the loop bound is derived correctly cold, and whether he can articulate why the bug didn't show up as a wrong test result. | ⏳ |
| LC 643 — Maximum Average Subarray I | Sliding Window — Fixed Size (numeric slide) | 2026-08-04 | 2026-08-25 — standard fuse. Clean, self-written, both naive and optimized versions brought unprompted, including his own division-at-the-end refinement. Correctly connected to `maxSubarraySumSizeK` fundamentals when asked directly (not spontaneously offered). At redo, check whether the fundamentals connection surfaces unprompted this time, and whether the naive complexity is labeled O(N·k) cold rather than O(N²). | ⏳ |
| LC 81 — Search in Rotated Sorted Array II | Binary Search — Rotated Array with Duplicates | 2026-07-26 | 2026-08-16 — standard fuse. Video-assisted origin, clean and correct on arrival, unprompted own-words explanation with correct core reasoning (only a minor Big-O labeling slip, corrected in the moment). At redo, check whether the O(n) worst-case complexity is stated correctly cold. | ⏳ |
| LC 33 — Search in Rotated Sorted Array | Binary Search — Rotated Array (sorted-half detection) | 2026-07-26 | 2026-08-16 — standard fuse. Video-assisted origin, but both real bugs (operator precedence in `mid`, `<` vs `<=` at `left===mid`) were found and fixed entirely through his own tracing, and the own-words explanation was given unprompted at wrap-up — ownership genuinely demonstrated live, not just claimed. At redo, check whether both bug shapes are avoided cold, not just whether the algorithm is remembered. | ⏳ |
| LC 189 — Rotate Array | Index Mapping via Modulo / Reversal / Cyclic Replacement | 2026-07-17 | 2026-07-27 ⚡ shorter fuse — mixed session: the extra-array solution and the core index-formula/aliasing insight were strong and mostly self-driven, but the modulo-wraparound concept AND both O(1)-space approaches (three reversals, cyclic replacement) were given as full direct teaching at his explicit request, not derived. At redo, require all three approaches attempted cold, with the two O(1)-space ones as the real test — don't accept "I remember roughly" without a working trace. | ⏳ |
| LC 2215 — Find the Difference of Two Arrays | Hash Set — Cross-Set Membership (Difference) | 2026-07-16 | 2026-08-06 — standard fuse. Clean, self-written, accepted (88.49th pct runtime), no bugs, unprompted own-words explanation given. Correctly identified as the mirror image of LC 349 without being told. | ⏳ |
| LC 349 — Intersection of Two Arrays | Hash Set — Cross-Set Membership (Intersection) | 2026-07-16 | 2026-08-06 — standard fuse. Clean, self-written, accepted (78.72nd pct runtime). One recurring minor bug not self-caught (`result` missing declaration keyword, patterns.md #10). Correctly reasoned the solution is already time/space optimal, and that the Set-based dedup step specifically depends on the duplicate-elements constraint. | ⏳ |
| LC 150 — Evaluate Reverse Polish Notation | Stack — Postfix Expression Evaluation | 2026-07-15 | 2026-08-05 — standard fuse. Near one-shot, only bug was `Math.floor` vs `Math.trunc` on negative integer division (patterns.md #70). At redo, check whether `Math.trunc` for truncation-toward-zero comes back cold without needing the failing trace again. | ⏳ |
| LC 155 — Min Stack | Stack — Auxiliary Min-Tracking Stack | 2026-07-15 | 2026-07-29 ⚡ slightly shorter fuse — video-assisted origin (disclosed), though engagement was genuinely strong (self-caught bug, self-derived the space-optimization mechanism). At redo, implement the space-optimized variant (lazy `minStack` pushes, `<=` comparison) that was deferred this session, not just the basic two-stack version. | ⏳ |
| LC 856 — Score of Parentheses | Stack — Score Accumulation | 2026-07-15 | 2026-08-05 — standard fuse. Clean, self-written, accepted, correctly and independently connected to the `nesting-score` fundamentals pattern without prompting — exact match, no adaptation needed. Declined the live own-words explanation at wrap-up (mood-dependent per his own stated preference, not pushed further) — fine to ask again at redo, accept whatever answer comes. | ⏳ |
| LC 394 — Decode String | Stack — Nested Group Decompression (marker-based, multi-digit) | 2026-07-15 | 2026-08-05 — standard fuse. Clean, self-written, accepted (100th pct runtime), correctly and independently connected to the `decompress-braces` fundamentals pattern without prompting, generalized to multi-digit counts. Declined to give the own-words explanation live at wrap-up ("you can explain it") — at redo, require the verbal walkthrough before/instead of writing it for him. Also revisit the two alternative approaches discussed but deferred (recursive descent, two-parallel-stacks) if time allows. | ⏳ |
