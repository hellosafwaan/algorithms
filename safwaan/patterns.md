...
- Transferring course-fundamentals patterns to real LeetCode problems unprompted — confirmed FOUR times in one session (2026-07-15): `reverseSomeChars`→LC 345, `befittingBrackets`→LC 20 (fully correct unprompted own-words explanation), `decompressBraces`→LC 394 (Medium, required generalizing to multi-digit counts, 100th percentile runtime), and `nestingScore`→LC 856 (Medium, exact match, zero adaptation). Holds across difficulty levels and both exact-match and generalized transfers — settled instinct, no longer needs per-instance verification.
  - The instinct also generalizes across *different* fundamentals modules, not just stack: at LC 643 (2026-08-04) he correctly connected the optimized approach to `maxSubarraySumSizeK` from the sliding-window fundamentals module — first confirmation outside stack. One caveat: this connection was made when asked directly ("does this ring a bell?"), not spontaneously offered the way the stack-module transfers were — worth a quiet check next time whether a sliding-window transfer surfaces unprompted.
...
- **Status:** Second occurrence across sessions, different specific mistake each time (n/2 vs O(n); O(N²) vs O(Nk)) but the same underlying shape — correct mechanism, imprecise final label. Probe again next time a nested loop's bounds aren't both `N`.
