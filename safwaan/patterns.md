...
### Course-fundamentals pattern transfer confirmed THREE times in one session — LC 345, LC 20, then LC 394 (2026-07-15)
Same session where he built a 5-problem stack fundamentals module (reverseSomeChars → nestingScore): LC 345 (Reverse Vowels) unprompted connected to `reverseSomeChars`; LC 20 (Valid Parentheses) unprompted connected to `befittingBrackets`, with a fully correct own-words explanation given unprompted; then LC 394 (Decode String, Medium — a real step up in difficulty from the first two) unprompted connected to `decompressBraces`, correctly identifying it as the same pop-until-marker/build-segment/repeat/push-back shape, generalized to handle multi-digit counts and different bracket characters. Accepted at 100th percentile runtime. This third instance is the strongest evidence yet — it held on a Medium-difficulty problem, not just Easy ones, and the underlying fundamentals pattern (`decompressBraces`) required more transformation to fit (multi-digit generalization) than the first two transfers did. Three different fundamentals patterns, three different real LeetCode problems, one sitting — see "What's Solid."
...
- Transferring course-fundamentals patterns to real LeetCode problems unprompted — confirmed THREE times in one session (2026-07-15): `reverseSomeChars`→LC 345, `befittingBrackets`→LC 20 (with a fully correct unprompted own-words explanation), and `decompressBraces`→LC 394 (Medium difficulty, required generalizing to multi-digit counts, 100th percentile runtime). Holds across difficulty levels, not just Easy problems.
