...
### "Recognition, not ownership" isn't just a video-assisted-solve risk — LC 2685 (2026-08-05)
Solved via a separate claude.ai chat session (not Claude Code), a heavily-scaffolded Socratic walkthrough of LeetCode's daily challenge (a directed-graph reachability problem). No video was involved — every step was guided by targeted questions in that session. Real, self-driven debugging did happen at several points (the undirected-graph slip, the infinite-loop trace). But after all tests finally passed, he said outright: "I genuinely did not understand shit lol." This is the same disengagement signature previously logged only for video-assisted solves (LC 200, LC 3169) — proof that "recognition, not ownership" isn't specifically a video-solve risk; it can happen from *any* assistance mode heavy enough on direct answers and leading questions, once enough individual pieces get handed over (here: two "tell me straight" direct fixes, plus several bugs pointed out rather than self-found). The session ended well — the other Claude gave a full clean walkthrough and explicitly invited a spoken re-explanation as the real test — but whether that re-explanation happened, and whether it actually landed, isn't visible in the shared transcript. Open thread: probe this directly next session, and treat it with the same shortened-revisit-fuse protocol as a video-assisted solve until ownership is actually demonstrated.

### Independent negative-number edge-case discovery kills a two-pointer approach — LC 560 (2026-08-04)
Brought a two-pointer "shrink while sum > k" attempt at Subarray Sum Equals K. Through guided tracing, self-diagnosed two real bugs in order: (1) the equality check happened before the shrink instead of after, missing windows that only became valid mid-shrink; (2) with the fix applied and a guard added, he then constructed and ran his OWN LeetCode test case (`nums = [-1,-1,1], k = 0`) — not one given to him — and it failed (returned 0, expected 1). Traced it himself to the actual root cause: an empty window (`start > end`) defaults to `sum === 0`, producing a false match whenever `k === 0`. When then asked directly whether the (fixed) approach would hold up given the problem's negative-number constraint, correctly reasoned it wouldn't, and independently supplied a second failing trace (`nums=[3,-2,5], k=5`, tested here — not by him, but he'd already reached the right conclusion before being shown it) demonstrating the sum-monotonicity assumption breaks entirely with negatives. This directly counters "Watch for: can skip edge case analysis unless pushed" — here the edge case was self-generated on the actual submission platform, not surfaced by the coach. Ultimately chose to scrap the problem rather than push through to prefix-sum + hashmap same session (see [[carry-forward]] for the deferred thread) — a reasonable call given the concept (prefix sums as a substitute for a monotonic window) is genuinely new, not a variant of anything in his sliding-window toolkit yet.
...
- Transferring course-fundamentals patterns to real LeetCode problems unprompted — confirmed FOUR times in one session (2026-07-15): `reverseSomeChars`→LC 345, `befittingBrackets`→LC 20 (fully correct unprompted own-words explanation), `decompressBraces`→LC 394 (Medium, required generalizing to multi-digit counts, 100th percentile runtime), and `nestingScore`→LC 856 (Medium, exact match, zero adaptation). Holds across difficulty levels and both exact-match and generalized transfers — settled instinct, no longer needs per-instance verification.
  - The instinct also generalizes across *different* fundamentals modules, not just stack: at LC 643 (2026-08-04) he correctly connected the optimized approach to `maxSubarraySumSizeK` from the sliding-window fundamentals module — first confirmation outside stack. This one needed a direct prompt ("does this ring a bell?"). The very next two problems in the same session, LC 1343 and LC 1456, both got the connection offered **unprompted**, before any code was even shown — "very similar to the last problem, just a small condition change" (1343) and "pretty similar to the last three problems... except this is a string problem" (1456). Three fixed-window sliding-window problems in one day, one prompted then two unprompted — the sliding-window instance of this instinct looks like it's settling the same way the stack one did, just needed one nudge to kick off.
...
- **What happened:** Declared `left` and `right` as `const` — can't decrement/increment them. In 977, also declared `right` without any keyword (implicit global). Recurred again at LC 349 — `result = []` with no `const`/`let`, also an implicit global. Recurred a fourth time at LC 567 — `for(key of mapB.keys())` inside `isSameMap`, missing `let`/`const` on the loop variable, an implicit global. Recurred a fifth time at LC 2685 (Remove Methods From Project, 2026-08-05, solved via a separate claude.ai session) — `[a, b] = edge;` with no declaration keyword. All five instances "worked by accident" or were flagged before submission — none were self-caught; the LC 2685 instance was pointed out directly by the coaching session, not found by him at all.
- **How it was caught:** Reminded to look at variable declarations each time — two-plus months later (LC 2685), still not self-caught before submission or before asking for help, and this time not caught even after being prompted to review the code (had to be pointed at the exact line).
- **Status:** Recurring across a two-plus-month span, different variable name each time (`left`/`right`, `right`, `result`, `key`, `[a, b]`) — a general "declared a variable without a keyword" habit, not tied to one specific name, context, or even destructuring vs. simple assignment. Still catches instantly when pointed out, never proactively. Worth a pre-submission checklist item, same recommendation as the `const`-reassignment pattern (#58).
...
### 80. Same root-cause bug recurs from a different trigger, minutes after being fixed
- **Seen in:** LC 2685 (Remove Methods From Project, 2026-08-05, solved via a separate claude.ai session)
- **What happened:** Fixed a `graph[node]` returning `undefined` bug (a target-only "leaf" node never got a `graph` entry) by seeding both `a` and `b` in `buildGraph`. Minutes later, a *different* trigger produced the exact same symptom: with a completely empty `invocations` array, `k` itself never appears anywhere, so it never gets seeded either — `dfs`'s `graph[currentNode]` was `undefined` again. This wasn't recognized as the same root cause; needed the direct fix given ("what's the fix, tell it to me straight").
- **Root cause:** The first fix was scoped to the specific trigger encountered (a node that's only ever a target), not to the general principle (any node might have zero `graph` entries, for any reason — never appearing as source, target, or existing in `invocations` at all). A narrower fix (seed harder in `buildGraph`) was applied instead of a fully general one (guard at read-time in `dfs` with `graph[node] || []`).
- **How it was caught:** Not self-diagnosed — the coach in that session pointed out it was the same underlying issue as before, then gave the direct fix after he asked for it straight.
- **Status:** First clean instance of "fixed a bug's specific trigger, not its general root cause, then hit a different trigger of the same root cause shortly after." Distinct from simple bug recurrence (patterns.md #10) — this is about the *generality* of a fix, not just remembering to apply it. Worth asking, next time a fix is found: "does this fix the specific case, or the general problem?"

### 79. Owns the window mechanics solidly but can't yet recognize a disguised window problem
- **Seen in:** LC 1658 (2026-08-05)
- **What happened:** Explicitly self-diagnosed before attempting: "I've solved so many sliding window problems, but I really don't know how to apply sliding window here... even after solving these many problems." LC 1658 asks about removing elements from the two ends of an array to hit a target sum — nothing about the phrasing suggests "contiguous subarray" the way most sliding-window problems do. The reframing (operations only touch the ends → the remainder is always contiguous → minimizing removed = maximizing a kept subarray summing to `totalSum - x`) needed four sequential guided questions. Once reframed, he wrote the entire standard variable-window implementation independently, correctly, with only one small self-caught bug (a leftover placeholder `return -1`).
- **Root cause:** This is a *different* skill from window mechanics, which are clearly solid (seven problems this week, mostly independent execution). It's specifically about recognizing when a problem's surface framing (removal, deletion, budget) is the complement of a contiguous-subarray problem — a translation step that has to happen before pattern-matching to "sliding window" can even start.
- **How it was caught:** Self-reported directly, unprompted, before any guided questions began — strong metacognitive accuracy about exactly where his own gap was (the reframing, not the execution).
- **Status:** First clean instance of this specific gap being named. Distinct from "abstract-to-code bridge" (patterns.md, various) — that's about turning an *understood* plan into code; this is about recognizing the plan is applicable at all. Probe on the next problem where the natural framing doesn't obviously look like sliding window (deletions, replacements, budget constraints).

### 78. Writing a known pattern from scratch is harder than adapting nearby code, even same-day
- **Seen in:** LC 3 cold redo vs. LC 438 (both 2026-08-04)
- **What happened:** At LC 438, he adapted LC 567's just-written solution directly — renamed variables, extended the "on match" behavior, and needed essentially no scaffolding (the one bug was a copy-paste artifact, patterns.md #76, unrelated to the sliding-window mechanism itself). An hour or two later, redoing LC 3 cold from a blank file — using the *identical* frequency-count + while-shrink mechanism he'd already used successfully that same day on LC 1343 and LC 1456 — he opened stuck ("I'm confused... unable to proceed") and needed several rounds of guided questions to rebuild the whole thing: what to store, when to add, which element to decrement, where to place the return.
- **Root cause (working theory):** Adapting existing code gives a structural scaffold for free — the shape is already on the screen, only specific pieces need changing. Writing from a genuinely blank file requires assembling that same structure from memory, which is a harder retrieval task even when the underlying mechanism was just successfully used minutes earlier on a different problem.
- **How it was caught:** Not "caught" so much as noticed by contrast — both sessions happened back to back on the same day, making the gap visible.
- **Status:** First clear instance of this specific contrast. Worth testing directly: does a *blank-file* cold start get easier as the same mechanism gets reused more times, independent of whether a revisit-queue problem is involved? This is somewhat different from the general "abstract-to-code bridge" gap already tracked — this is specifically about blank-file recall vs. adapt-existing-code recall of a mechanism he already owns.

### 77. Cold revisit forgets a lesson that's explicitly written in the problem's own notes
- **Seen in:** LC 209 cold redo (2026-08-04, original solve 2026-06-24)
- **What happened:** First real completed attempt from the revisit queue (past-due since 2026-07-15). Brought a fresh solution with two bugs: (1) `start` never incremented inside the shrink `while` loop — self-caught in one question; (2) validity checked *after* the shrink instead of *during* it, meaning the window's length only got recorded when the sum happened to land exactly on `target`, missing every case where a valid window (`sum >= target`) existed mid-shrink but got shrunk past before anything checked it. Bug (2) is the *exact* lesson already spelled out in this same problem's `learnings.md`, under "Watch Out For": "Record inside the while loop, before shrinking. If you record after the while loop exits, the window is already invalid." He'd written that himself in June and it didn't surface on the August cold redo. Needed the direct answer for this one, not just a nudge — tried an intermediate fix (`>= target` instead of `=== target`, still checked after the loop) that didn't work, then got stuck on why it didn't work, then received the answer directly.
- **How it was caught:** Traced a specific failing case (`target=11, nums=[1,2,3,4,5]`, expected `3`, got `0`) step by step through the shrink, confirming the valid window (`sum=12`) was skipped.
- **Status:** First data point for how the revisit-queue system actually performs when a redo is past due and genuinely attempted (rather than deferred/avoided, which was the more common outcome in July). The result: partial retention — bug 1 (a structural/mechanical slip) was caught fast and independently; bug 2 (a subtler *why*-level insight about loop-exit invariants) needed to be re-taught, even though it was already documented. Given a fresh revisit date rather than marked "Done." Worth watching whether a *third* attempt (without a guided walkthrough) retains it, which would be the real test of whether being re-taught once was enough.
...
- **Root cause:** The mechanism (nested loops → multiply the bounds) is understood; the slip is treating "two nested loops" as a reflex shortcut to O(N²) rather than actually multiplying the two specific bounds involved.
- **How it was caught:** Asked to look at the two loops' actual bounds (`N - k` and `k`) side by side and multiply them, rather than being told the answer — self-corrected to O(Nk) in one step.
- **Status:** Three occurrences now across sessions (LC 81, LC 643, and LC 1456 same-day — see below), same underlying shape each time: correct mechanism, imprecise final label. The correction is getting faster and more confident, though — at LC 1456 he simplified a non-standard "O(N-K)" time label to O(N) and corrected an O(k)-labeled space complexity to O(1) (recognizing the vowels `Set` is fixed-size, unrelated to `k` or `n`) both in a single beat each, no back-and-forth needed ("I just O of N, man"). Looks like the "check the actual bounds/whether a structure's size really depends on the input" nudge is starting to work as a fast self-check rather than needing full re-derivation each time.
