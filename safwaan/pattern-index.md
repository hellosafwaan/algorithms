...
| LC 3169 — Count Days Without Meetings *(bonus)* | Classify-and-merge, then a gap-summation pass over the result | Merge first (identical to LC 56), then sum free days: before first (`result[0][0]-1`), between consecutive pairs (`next[0]-current[1]-1`, note the `-1` — adjacent-but-non-overlapping intervals share zero free days), after last (`days-result[last][1]`). Push must be an `else`, not unconditional — merging replaces, it doesn't also push. |
