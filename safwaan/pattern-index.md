...
| LC 81 — Search in Rotated Sorted Array II *(bonus)* | Rotated array with duplicates | Same as LC 33, plus one guard: when `nums[left] === nums[mid] === nums[right]`, the sorted-half comparison is uninformative — shrink both ends by one (`left++`, `right--`) instead of halving. Degrades worst case to O(n) (e.g. all-identical array), best/average case stays O(log n). |
...
| LC 1456 — Maximum Number of Vowels in a Substring of Given Length *(bonus)* | Fixed window, Set-membership count slide | Third fixed-window problem in the same streak, unprompted self-connected before any code was shown. `Set` for O(1) vowel lookup instead of `string.includes()` (O(k) per check). Otherwise identical seed-then-slide shape — window tracks a membership count instead of a sum. Vowels `Set` is O(1) space (fixed 5 chars), not O(k) — self-corrected when asked whether the set's size actually depends on the input. |
| LC 567 — Permutation in String | Fixed window, exact-frequency Map match | Curriculum problem #4, first non-bonus sliding-window problem of the day. Unprompted connected to the fundamentals anagram problems, correctly reasoning duplicates rule out a Set and require a Map — though named `has-substring-anagram` (Set-based, #4) when the actual structural match is `count-substring-anagrams` (Map-based, #5). Algorithm was correct from the start (verified against 20,000 random tests); the only real issue was a silent implicit-global bug in the `isSameMap` helper (`for(key of ...)` missing `let`, patterns.md #10, 4th occurrence). |
| LC 438 — Find All Anagrams in a String *(bonus)* | Fixed window, exact-frequency Map match (multi-result) | Direct extension of LC 567 (unprompted self-connected), collecting every matching start index (`i + 1` after each slide) instead of returning on the first. Algorithm correct once relocated — the actual bug was a copy-paste artifact, real logic left under the leftover function name `checkInclusion` while the required `findAnagrams` sat empty (patterns.md #76, new bug shape). No repeat of the LC 567 implicit-global bug. |
| LC 1658 — Minimum Operations to Reduce X to Zero *(bonus)* | Variable window, complement reframing | The window mechanics were already owned solidly; the actual gap was recognizing the problem *as* a window problem at all. Key move: operations only remove from the two ends, so the remainder is always contiguous — minimize removed ⟺ maximize a contiguous chunk summing to `totalSum - x`. Once reframed, wrote the full standard variable-window implementation independently (one small self-caught bug: leftover placeholder `return -1`). See `sliding-window/notes.md` "Recognizing Disguised Problems." |
| LC 1493 — Longest Subarray of 1's After Deleting One Element *(bonus)* | Variable window, at-most-one-exception | Reframed "delete one element to maximize a run of 1s" into "longest window with at most one `0`," derived entirely via guided questions with zero direct answers given. Answer is `windowLength - 1`, which already covers the all-1s edge case (must still delete something) without a special check. One bug: gating the record step behind `zeroCount === 1` excluded the zero-zero case even after correctly reasoning through it verbally — fixed by recording unconditionally, since the post-shrink window is always already valid. Same mechanism as `fundamentals/10-max-ones-with-single-flip` — connected at wrap-up (after the solve): flip-in-place keeps the window length, delete shrinks it by one, otherwise identical. |
| LC 713 — Subarray Product Less Than K *(bonus)* | Variable window, count-all-valid-subarrays | Unprompted self-connected to `fundamentals/11-count-subarray-product` before any code shown — direct transfer, same technique. Real bug: shrink loop missing `start <= end` guard, allowing `start` to overshoot `end` and corrupt the count with negative values (`nums=[7,2,7,8,6], k=1` → `-4` instead of `0`). Found via stress testing, not self-caught — and the buggy version had already been Accepted by LeetCode, exposing a gap in the official test suite. |
