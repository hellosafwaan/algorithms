...
- **Big-O labeling can be imprecise even when the underlying mechanism is correctly understood** — two instances so far: LC 81 (2026-07-26, said "n/2" instead of "O(n)") and LC 643 (2026-08-04, said O(N²) for a naive nested loop actually bounded by `N` and `k`, i.e. O(N·k), which only degenerates to O(N²) when `k` scales with `N`). Both times, a nudge to look at the actual bounds/convention produced an immediate self-correction — this is a labeling-precision gap, not a reasoning gap. Prompt: "what are the actual bounds here?" before accepting his first-stated Big-O.
