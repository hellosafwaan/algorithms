...
- Transfers course-fundamentals patterns to real LeetCode problems unprompted — built a 5-problem Stack Fundamentals module (course material, not NeetCode 150), then independently solved LC 345 (Reverse Vowels of a String) in the same sitting and unprompted named it as the same push/pop-reversal pattern as the fundamentals problem he'd just built (2026-07-15). One data point so far — worth testing whether it holds once there's more time distance from the fundamentals work, or on a second pattern (e.g. bracket matching → LC 20).
